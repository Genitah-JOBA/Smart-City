    ...
    const fetchUserSignalements = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setMsg({ type: "error", text: t("agent.cantLoadReports") });
        }
      } catch (error) {
        console.error(error);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4">
      ...
      <div className="container mx-auto max-w-4xl pt-8">
        <h1 className="text-3xl font-bold text-white mb-6">📋 {t("prof.myReports")}</h1>
        {signalements.length === 0 ? (
          <div className="bg-white/10 rounded-2xl p-12 text-center">
            <p className="text-white/70">{t("mine.empty")}</p>
          </div>
        ) : (
            ...
            {signalements.map(s => (
              <div key={s.id} className="bg-white/10 rounded-xl p-4">
                ...
                <div className="flex items-center gap-2 text-white/40 text-xs mt-2">
                  <MapPin size={12} />
                  <span>{s.address || s.ville || t("mine.noPosition")}</span>
                </div>
              </div>
            ))}
          </div>
